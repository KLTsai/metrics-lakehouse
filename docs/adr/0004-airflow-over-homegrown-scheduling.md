...
Airflow 把拆在三處的東西收回一個地方:DAG 檔案。本 repo 的 `drive_ingestion` DAG(`airflow/dags/drive_ingestion.py`,T1.2–T1.5 四個 commit `ad818e2`→`87d29df` 逐步長成)就是實例——什麼時候跑(`@daily`)、步驟的先後順序(下載 → 驗表頭 → 入倉)、每一步失敗要不要重試、失敗要不要發通知,全部寫在同一個檔案裡,而且進版本控制;只有收件人地址與 SMTP 連線走環境變數,不把信箱寫死進程式碼。接手的人讀這個檔案,就是在讀排程的全部真相。
...
2. **漏掉的補得回來。** Airflow 給每一次執行一個明確的日期,叫 logical_date;本專案約定「logical_date 是 D 的那次執行,負責檔案日是 D 的檔案」(詞條見 CONTEXT.md)。停機幾天後恢復,`catchup=True` 會把落掉的日期一天一天自動補跑;本 repo 的 DAG 另設了 `end_date=2026-01-05`,把窗口固定成 5 趟示範資料,所以補跑能力是靠這 5 趟與手動 backfill 驗證的,不是靠長期無人值守運轉。想重跑某一天,指定那個日期就行——配合分區替換(ADR 0002),重跑多少次結果都一樣。
