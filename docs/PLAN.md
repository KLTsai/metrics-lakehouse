...
| T0.5 | 髒資料產生器 v1：參考藍本 schema（訂單、應收帳款），生成多租戶 CSV；髒資料注入可設比例（缺值/重複/格式漂移/遲到） | 指定 5% 髒資料率時，實際輸出可驗證吻合；檔案上傳至 Drive |
...
| 風險 | 緩解 |
|---|---|
| **容器記憶體配額偏緊**（Colima VM 4 GiB，正好卡在 Airflow 官方下限） | 舊風險「RAM 16GB 偏緊」的依據已隨換機失效（原文為 Ryzen 7 4800U 機器 + `.wslconfig` 8GB 上限的觀測）。**真正的天花板不是 host RAM，是 Colima VM 配額**：實測 Apple M4 / 10 核 / 16GB（free 40%，host 不是瓶頸）、磁碟餘 377GB，但 `colima list` 顯示 VM 僅 **4 GiB**（容器可見 3.813 GiB）——**比舊機 `.wslconfig` 的 8GB 上限更小**。Airflow 官方文件（docker-compose 頁）明載「至少 4GB、macOS 理想 8GB」，並點名 macOS 預設配額常不足、症狀是 **webserver 反覆重啟**。但該數字針對 CeleryExecutor；T1.1 明訂 LocalExecutor、不跑 Celery/Redis，實際需求應更低（低多少無官方數字——**這正是 T1.1 驗證標準「記憶體佔用有量測紀錄」要產出的東西**）。Spark 不佔本機（L4 在 Databricks 雲端跑），4 GiB 不必為 Phase 3 預留。**緩解**：維持 4 GiB 進 T1.1，讓量測有意義；一旦出現 webserver 反覆重啟即判定為配額不足，`colima stop && colima start --memory 8` 一行解決（host 16GB 有餘裕）。docker 資料碟是獨立的 Lima external disk，機制上改配額不影響既有 volume（**未實測**——保險起見調整前先 `pg_dump`） |
